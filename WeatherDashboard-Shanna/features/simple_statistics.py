...
class SimpleStatsPanel:
    ...
    def create_stats_interface(self):
        title_label = ttk.Label(self.main_frame, text="📊 Weather Statistics Dashboard", font=("Segoe UI", 20, "bold"))
        ...
        ttk.Label(input_grid, text="State:").grid(row=1, column=0, sticky='w', padx=(0, 5))
        self.state_entry = ttk.Entry(input_grid, width=20)
        self.state_entry.grid(row=1, column=1, padx=(0, 20))
        ...
        refresh_btn = ttk.Button(buttons_frame, text="🔄 Refresh Stats", command=self.refresh_stats)
        refresh_btn.pack(side='left', padx=(0, 10))

        export_btn = ttk.Button(buttons_frame, text="💾 Export Data", command=self.export_data)
        export_btn.pack(side='left')

        self.stats_frame = ttk.LabelFrame(self.main_frame, text="📊 Statistics", padding=20)
        self.stats_frame.pack(fill='both', expand=True)

        self.stats_text = tk.Text(self.stats_frame, height=20, width=80, font=("Consolas", 11), wrap='word')
